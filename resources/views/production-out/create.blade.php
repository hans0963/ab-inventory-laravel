<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-end border-b-2 border-sienna pb-4">
            <div>
                ...
                <p class="font-inter text-sage mt-1 font-medium uppercase tracking-wider text-xs">Record product removals from inventory</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="card-rustic border-sienna">
        <form action="{{ route('production-out.store') }}" method="POST">
            ...
            <!-- Basic Info -->
            <div class="mb-8">
                <h3 class="text-xl font-bold text-sienna mb-4 border-b border-sienna border-opacity-10 pb-2">Transaction Details</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <x-text-input id="reason" name="reason" type="text" placeholder="e.g., Repackaging, Quality Check" value="{{ old('reason') }}" required class="mt-1 block w-full !text-sm" />
            ...
            <!-- Items Section -->
            <div class="mb-8">
                <h3 class="text-xl font-bold text-sienna mb-4 border-b border-sienna border-opacity-10 pb-2">Items to Remove</h3>
            ...
            <div class="flex gap-4 pt-4 border-t border-sienna border-opacity-10">
                ...
                <a href="{{ route('production-out.index') }}" class="inline-flex items-center px-4 py-2 bg-cream border border-sienna border-opacity-20 rounded-md font-bold text-xs text-sienna uppercase tracking-widest hover:bg-opacity-50 transition">
                    Cancel
                </a>
            </div>
        </form>
