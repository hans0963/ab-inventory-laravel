<x-app-layout>
        ...
        <div class="flex justify-between items-center w-full bg-cream p-4 rounded-md shadow-sm border-b border-sienna">
            <div>
                <h2 class="font-formal text-3xl text-sienna leading-tight">
                    {{ __('Stock Out: Baking Ingredients') }}
                </h2>
                <p class="font-inter text-sage">Record usage or removal of raw materials</p>
            </div>
    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white rounded-2xl shadow-xl border-t-4 border-orange-600 p-8">
            <form action="{{ route('raw-materials.processOut') }}" method="POST">
                @csrf

                <div class="mb-8">
                    <label class="block text-sm font-semibold text-sienna mb-2">Reason for Removal *</label>
                    <input type="text" name="reason" required 
                           ...
                           placeholder="e.g. Daily usage for Pandesal batch, Spoilage, etc.">
                </div>

                <div class="mb-6">
                    <h3 class="text-lg font-lora font-bold text-sienna mb-4">Select Ingredients</h3>
                            ...
                            <div class="flex-1">
                                ...
                                <select name="material_id[]" required 
                                    ...
                                    @foreach($materials as $material)
                                        <option value="{{ $material->id }}" data-stock="{{ $material->quantity }}" data-unit="{{ $material->unit }}">
                                            {{ $material->material_name }} (Stock: {{ $material->quantity }} {{ $material->unit }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="w-full md:w-32">
                                <label class="block text-xs font-bold text-sage uppercase tracking-widest mb-1">Quantity</label>
                ...
                <div class="flex justify-end pt-6 border-t border-sienna border-opacity-10">
                    <button type="submit" 
                            class="bg-orange-600 hover:bg-orange-700 text-white font-black px-12 py-3 rounded-xl shadow-rustic transition-all hover:-translate-y-0.5 active:translate-y-0">
                        Process Stock Out
                    </button>
                </div>
            </form>
