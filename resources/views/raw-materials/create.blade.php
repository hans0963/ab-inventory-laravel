    ...
    <x-slot name="header">
        <div class="flex justify-between items-end border-b-2 border-sienna pb-4">
            <div>
                ...
                <p class="font-inter text-sage mt-1 font-medium uppercase tracking-wider text-xs">Register ingredients, packaging, and supplies</p>
            </div>
        </div>
    </x-slot>

    <div class="card-rustic border-sienna max-w-4xl mx-auto mt-8">
        <form method="POST" action="{{ route('raw-materials.store') }}" class="space-y-8">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    <x-input-label for="material_name" value="Material Name *" class="text-[10px] uppercase tracking-widest text-sage" />
                    <x-text-input id="material_name" name="material_name" type="text" value="{{ old('material_name') }}" required placeholder="e.g. Flour, Sugar, Cake Box" class="mt-1 block w-full !text-sm" />
                ...
                <div>
                    ...
                    <x-text-input id="unit" name="unit" type="text" value="{{ old('unit', 'kg') }}" placeholder="kg, g, L, pcs, box" class="mt-1 block w-full !text-sm" />
                ...
                <div>
                    ...
                    <x-text-input id="expiry_alert_days" name="expiry_alert_days" type="number" value="{{ old('expiry_alert_days', 7) }}" min="0" max="365" class="mt-1 block w-full !text-sm" />
                ...
                <div>
                    ...
                    <x-text-input id="reorder_level" name="reorder_level" type="number" value="{{ old('reorder_level', 0) }}" min="0" class="mt-1 block w-full !text-sm" />
                ...
                <div>
                    ...
                    <x-text-input id="reorder_quantity" name="reorder_quantity" type="number" value="{{ old('reorder_quantity', 0) }}" min="0" class="mt-1 block w-full !text-sm" />
