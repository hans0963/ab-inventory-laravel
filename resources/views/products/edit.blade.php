            ...
            <div id="recipe-section" class="space-y-5 pt-8 border-t border-sienna border-opacity-10">
                <div class="flex items-center justify-between gap-4">
                    ...
                    <label class="inline-flex items-center gap-2 text-xs font-black text-sage uppercase tracking-widest">
                        <input type="checkbox" name="recipe_enabled" value="1" class="rounded border-sienna text-sienna focus:ring-sienna" {{ old('recipe_enabled', $product->recipe ? 1 : 0) ? 'checked' : '' }}>
                        Track Recipe
                    </label>
                </div>
                ...
                <div id="recipe-items" class="space-y-4">
                    @php
                        $existingIngredients = $product->recipe
                            ? $product->recipe->ingredients->map(fn ($item) => [
                                'raw_material_id' => $item->raw_material_id,
                                'quantity_per_unit' => $item->quantity_per_unit,
                            ])->toArray()
                            : [['raw_material_id' => '', 'quantity_per_unit' => '']];
                        $oldIngredients = old('recipe_ingredients', $existingIngredients ?: [['raw_material_id' => '', 'quantity_per_unit' => '']]);
                    @endphp
                ...
                <div>
                    ...
                    <textarea id="recipe_notes" name="recipe_notes" rows="2" class="mt-1 block w-full border-sienna border-opacity-20 rounded-md shadow-sm focus:border-sienna focus:ring focus:ring-sienna focus:ring-opacity-50 text-sm">{{ old('recipe_notes', $product->recipe?->notes) }}</textarea>
                </div>
            </div>
