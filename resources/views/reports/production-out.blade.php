        ...
        <div class="flex justify-between items-end border-b-2 border-sienna pb-4">
            <div>
                <h2 class="font-formal text-4xl text-sienna">
                    {{ __('Production OUT Report') }}
                </h2>
                <p class="font-inter text-sage mt-1 font-medium uppercase tracking-wider text-xs">{{ $viewType === 'summary' ? 'Summary' : 'Detailed' }} view of wastage/pull-outs</p>
            </div>
                    ...
                    <tbody class="divide-y divide-sienna divide-opacity-10 bg-white">
                        @forelse($data as $row)
                            <tr class="hover:bg-cream hover:bg-opacity-20 transition">
                                <td class="px-6 py-4 font-bold text-sienna">{{ $row->production_out_no }}</td>
                                <td class="px-6 py-4 text-sage">{{ $row->date->format('M d, Y') }}</td>
                                <td class="px-6 py-4 text-sienna font-medium">{{ $row->reason }}</td>
                    ...
                    <tbody class="divide-y divide-sienna divide-opacity-10 bg-white">
                        @forelse($data as $row)
                            @foreach($row->items as $item)
                                <tr class="hover:bg-cream hover:bg-opacity-20 transition">
                                    <td class="px-6 py-4 font-bold text-sienna">{{ $loop->first ? $row->production_out_no : '' }}</td>
                                    ...
                                    <td class="px-6 py-4 text-sage italic text-xs">{{ $row->reason }}</td>
                                    <td class="px-6 py-4 text-right font-black text-terracotta">₱{{ number_format($item->total_value, 2) }}</td>
                                </tr>
                            @endforeach
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-sage italic">No production data found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
